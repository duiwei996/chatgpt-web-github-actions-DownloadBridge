...
function drawFallback(name,x,y,w,h,flip=false,alpha=1){
 const icons={slime:'🟢',bat:'🦇',skeleton:'💀',skeleton_shield:'🛡️',mushroom:'🍄',ghost:'👻',bee:'🐝',boar:'🐗',boss_mushroom:'🍄',boss_tree:'🌳',fireball_fx:'🔥',ice_fx:'❄️',lightning_fx:'⚡',leaf_fx:'🍃',blade_fx:'🌀',skill_fire:'🔥',skill_meteor:'☄️',skill_ice:'❄️',skill_lightning:'⚡',skill_heal:'💚',skill_shield:'🛡️',skill_leaf:'🍃',skill_blades:'🌀',skill_laser:'💫',skill_poison:'☠️',skill_tornado:'🌪️',skill_spirit:'👻',item_coin:'🪙',item_gem:'💎',item_chest:'🎁',item_potion:'🧪',item_magnet:'🧲',item_boots:'👢',item_crown:'👑',item_armor:'🛡️',item_revive:'🪶',item_wand:'🪄'};
 ctx.save();ctx.globalAlpha=alpha;ctx.translate(x,y);if(flip)ctx.scale(-1,1);
 const size=Math.max(18,Math.min(w||64,h||w||64));
 if(name.startsWith('hero_')){const hurt=name==='hero_hurt',win=name==='hero_win';ctx.fillStyle='#2b3946';ctx.beginPath();ctx.ellipse(0,size*.25,size*.28,size*.22,0,0,TAU);ctx.fill();ctx.fillStyle='#ff554f';ctx.beginPath();ctx.moveTo(-size*.32,-size*.02);ctx.lineTo(size*.28,size*.03);ctx.lineTo(size*.44,size*.17);ctx.lineTo(-size*.25,size*.13);ctx.closePath();ctx.fill();ctx.fillStyle='#ffe6ce';ctx.beginPath();ctx.arc(0,-size*.18,size*.3,0,TAU);ctx.fill();ctx.fillStyle='#f6eee3';for(let i=-2;i<=2;i++){ctx.beginPath();ctx.arc(i*size*.09,-size*.38+Math.abs(i)*size*.025,size*.13,0,TAU);ctx.fill();}ctx.fillStyle='#303a48';ctx.fillRect(-size*.28,-size*.46,size*.56,size*.07);ctx.fillStyle='#58baf2';ctx.beginPath();ctx.arc(-size*.13,-size*.43,size*.08,0,TAU);ctx.arc(size*.13,-size*.43,size*.08,0,TAU);ctx.fill();ctx.fillStyle=hurt?'#ff6464':'#24527a';ctx.font=`900 ${size*.17}px sans-serif`;ctx.textAlign='center';ctx.fillText(hurt?'× ×':win?'^ ᵕ ^':'• ᵕ •',0,-size*.13);if(win){ctx.font=`${size*.28}px sans-serif`;ctx.fillText('✨',size*.34,-size*.25);}}
 else{ctx.font=`${size*.78}px "Apple Color Emoji","Segoe UI Emoji",sans-serif`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(icons[name]||'✦',0,0);}
 ctx.restore();
}
